...
const UpdateReqForm = (props) => {
   ...
   //  useEffect(()=>{
   //   props.req.author === user.email ? setPermission(true) : setPermission(false)
   //  },[])
    ...
    const handleSubmit = (e) => {
        ...
        axios.post(`${process.env.REACT_APP_URL}/requests/update`, {
            _id: request._id,
            name: request.name,
        ...
        <>
           <div className="m-3 p-5 flex flex-col border-2">
              <form onSubmit={handleSubmit}>
                 <div className="flex items-center my-3"><h4 className="w-11/12">UPDATE REQUEST</h4><button type="submit" className="bg-green-500 text-slate-50 px-3 py-1 shadow-lg">Update</button></div>
                 ...
                 <div className="my-2 border-2 p-4 flex flex-wrap">
                    ...
                    <div className="w-1/2"><span>Status</span>
                    <select list="status" name="status" className="border-2 mx-3" onChange={handleChange} value={request.status} >
